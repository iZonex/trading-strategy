...
- **OI loading in silence** $\rightarrow$ barrel loaded $\rightarrow$ pump coming (Model 1)
- **OI loading in volatility** $\rightarrow$ smart money masking entry $\rightarrow$ pump coming (Model 6)
- **OI growing during active pump** $\rightarrow$ smart money still adding $\rightarrow$ pump continues (Model 7)
- **OI dropping in silence** $\rightarrow$ whale exiting quietly $\rightarrow$ dump coming (Model 2)
- **OI dying after pump peak** $\rightarrow$ fuel exhausted $\rightarrow$ dump coming (Model 3)
- **OI growing on price dip** $\rightarrow$ absorption (buying the dip) $\rightarrow$ recovery coming (Model 4)
- **OI stabilizing after dump** $\rightarrow$ cascade over $\rightarrow$ bounce coming (Model 5)
...
Total data: 3.3 GB across 246 coins. Primary analysis on 14 coins with OI-price divergence $\geq 40\%$; extended testing on all 246.

### 2.2 Derived Metrics

**OI-Price Divergence Rate** (the master filter):

$$D(T) = \frac{\#\{h \in T : \text{sign}(\Delta P_h) \neq \text{sign}(\Delta \text{OI}_h)\}}{\#\{h \in T\}}$$

where $h$ indexes 1-hour windows over period $T$, $\Delta P_h$ is the price change and $\Delta \text{OI}_h$ is the OI change in window $h$.

**OI Predictive Power** (grey-zone filter):

$$\text{Pred}(T) = \frac{\#\{h \in T_{\text{div}} : \text{sign}(\Delta P_{h+1}) = \text{sign}(\Delta \text{OI}_h)\}}{\#\{h \in T_{\text{div}}\}}$$

where $T_{\text{div}} \subset T$ is the set of divergent hours (where OI and price moved opposite). If $\text{Pred} \geq 52\%$, the divergent OI correctly predicts the next hour's price---indicating smart money, not noise.
...
*Figure 1: OI-price divergence rate as the universal coin filter. Coins with divergence $\geq 40\%$ (green) produce +832% over 260 barrel trades. Coins below 40% (red) produce -1,232% on the same signals. FARTCOIN (yellow) has high divergence but fails the OI Predictive Power secondary filter.*
...
On coins with low divergence ($< 30\%$), OI and price move together: price up $\rightarrow$ FOMO longs (OI up), price down $\rightarrow$ liquidations (OI down). This is pure retail momentum. No one stands against the crowd. Barrel triggers on these coins are random noise.
...
**Combined filter:** Divergence $\geq 40\%$ AND Predictive Power $\geq 52\%$ eliminates all grey-zone false positives.
...
**WHAT:** OI grows 5--30% while price ranges $<4\%$/h. Then a trigger bar breaks the silence with $5\times$+ volume.
...
**WHERE:** 7-day uptrend. BTC flat or down ($\text{BTC}_{4\text{h}} < +2\%$). When BTC pumps, money flows to BTC, not shitcoins.

**Results:** 188 trades on 12 Div$\geq$40% coins. WR 73%, PF 3.1, +663%. Quality filter (vol$\geq$8$\times$, bar$\geq$2%, OI$\geq$8%): N=28, PF 14.2, DD 22%.
...
**Results:** Diamond hands coins only (Div$\geq$40%): N=46, **WR 87%, PF 21.3**, +177%, DD 4.6%.
...
**WHY:** Smart money sees value at lower prices. Their buying (OI up) against price decline = absorption. Larger OI growth = stronger signal: OI +12% at dip $\rightarrow$ avg +1.16%/trade (vs +0.40% at OI +2%).
...
**Critical difference from Model 1:** TopLong $\geq 50\%$ = *better* on volatile barrel (PF 4.91 vs 1.68). Opposite of silent barrel. When longs dominate AND OI grows through volatility, this is conviction building.

**Results:** 136 trades, Div$\geq$40% coins. WR 72%, PF 2.97, +522%. Quality filter: N=59, PF 4.45, DD 16%.
...
**Results:** Best combo (OI$\uparrow >3\%$ + vol accelerating): N=85, **WR 81%, PF 13.8**, +12.9%/trade. Beats random P95.
...
1. **Silent dump** (SWARMS pattern): silence $\rightarrow$ sudden drop. But OI does NOT load before. No accumulation visible. Unpredictable with OI data.
...
Liquidation map (Sliq2%) correctly identified JOE pump exhaustion in real-time (Sliq2% shorts = \$22K $\approx 0$ $\rightarrow$ pump dead). But backtest improvement was only 2--5pp WR. Cause: leverage distribution is estimated (15/25/30/20/10%), not real. **Liq map useful for extreme cases, not universal filter.**
...
|---------------|---|----|----|
| Down $< -2\%$ | 8 | **88%** | **33.8** |
| Flat $\pm 0.5\%$ | 117 | 76% | 2.9 |
| Up $> +2\%$ | 12 | **42%** | **0.69** |

Counter-intuitive: barrel works BEST when BTC dumps. Money rotates: BTC down $\rightarrow$ traders seek alt gains $\rightarrow$ shitcoin pumps are real. When BTC pumps, everyone is in BTC---shitcoin triggers are noise.

**Filter:** Skip barrel if $\text{BTC}_{4\text{h}} > +2\%$.
...
|-------|-----------|-------------|
| DEAD | 51% | Range $< 2\%$, volume $< 0.5\times$ avg |
| QUIET | 30% | Range $< 3\%$ |
| NORMAL | 14% | Range 3--5% |
| VOLATILE | 4% | Range $> 5\%$ |
| PUMP | **1%** | $> +5\%$/h |
...
|------|---|--------|----------|
| Barrel Pump | 10 | Model 1 | $\checkmark$ |
| Silent Dump | 26 | Model 2 | $\checkmark$ |
| Pump-then-Dump | 11 | Model 3 | $\checkmark$ |
| FOMO Pump | 41 | Model 4 (partial) | $\checkmark$ |
| Dump-then-Pump | 12 | Model 5 | $\checkmark$ |
| Volatile Pump | ~25 | Model 6 | $\checkmark$ |
| Continuation | ~35 | Model 7 | $\checkmark$ |
| Cascade Dump | 9 | Not reliably modelable | $\times$ |
| Chaotic | 13 | Noise | $\times$ |
...
**Sweet spot: TopLong 40--50%.** Balanced crowd $\rightarrow$ trigger = genuine breakout, not squeeze or FOMO. PF 4.2.
...
Two correct calls from 8 candidates. Both Div$\geq$40% coins.
...
- JOE: Sliq2% shorts = \$22K $\approx 0$ (no fuel) $\rightarrow$ reject
- All 5: entered during US session at 14:44 UTC (worst session) $\rightarrow$ deprioritize
- ARIA: pump already +34% $\rightarrow$ too late without OI confirmation
...
1. OI-price divergence $\geq 40\%$ (93% accuracy)
2. OI Predictive Power $\geq 52\%$ (kills false positives like FARTCOIN)

**Trigger Conditions (real-time):**

3. Model-specific entry (barrel, dip-buy, bounce, continuation, etc.)
4. Trigger quality: vol $\geq 8\times$, bar $\geq 2\%$, top wick $< 20\%$
5. Session: prefer 0--13 UTC (Asia/EU). Skip US afternoon.
6. BTC context: skip if $\text{BTC}_{4\text{h}} > +2\%$
...
- $< 40\%$: Short squeeze $\rightarrow$ wider trail (activate +8%), hold longer
- 40--55%: Balanced $\rightarrow$ standard trail
- $> 60\%$: Demand-driven $\rightarrow$ tight trail (+3%) or skip
